...
sf::Vector2f translate(sf::Vector2f posDis, Container& ds)
{
  ...
  sf::Vector2f middleDis(ds.get_dw()/2.0, ds.get_dh()/2.0);
  sf::Vector2f middleTex(ds.get_tw()/2.0, ds.get_th()/2.0);
  double theta = M_PI/double(ds.get_NoAxis());
  ...
  return sf::Vector2f(radiusT*cos(ds.get_angle()+alpha), radiusT*sin(ds.get_angle()+alpha))+middleTex;
}
...
sf::Color interpolateBL(sf::Vector2f pos, Container& ds)
{
  ...
  x1 = x1<0 ? 0 : x1;
  y1 = y1<0 ? 0 : y1;
  x1 = x1>=ds.get_tw() ? ds.get_tw()-2 : x1;
  y1 = y1>=ds.get_th() ? ds.get_th()-2 : y1;
  int x2 = x1+1;
  int y2 = y1+1;
  ...
  a11 = ds.getPixel(x1, y1);
  a12 = ds.getPixel(x2, y1);
  a21 = ds.getPixel(x1, y2);
  a22 = ds.getPixel(x2, y2);
